...
angular.module("app").controller("matchStatsCtrl", function ($scope, $stateParams, mainService, $rootScope) {
    ...
    //Full match graph
    var canvasMatch = d3.select(".match-graph-cont-complete").append("svg").attr("width", width).attr("height", height).attr("style", "border: 1px solid red");
    ...
    }).attr("height", 50 / ($scope.game.selectMatch / 2)).attr("y", function (d, i) {
        return i * (50 / ($scope.game.selectMatch / 2));
